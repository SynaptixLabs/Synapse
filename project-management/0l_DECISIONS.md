...
| D-5 | 2026-07-15 | **Graph schema v2: `pathref` edges** — backticked `` `*.md` `` path pointers resolve to edges (note-relative, then repo-root-relative); unresolvable code mentions are NOT recorded as unresolved | Founder asked who defines connectivity and why agent adapters sat isolated: our own thin-adapter doctrine points via code paths, which rule "code ≠ link" ignored — the brain couldn't see the agent-layer wiring | yes (rebuildable; schema versioned) | cpto, founder-raised |
| D-6 | 2026-07-15 | **Source roots are UI-managed (CRUD)** — `data/roots.json` (add / enable-disable / remove-with-prune) wins over `SYNAPSE_SOURCE_REPOS` env (seed only); default with nothing configured = **this project itself** | Founder ruling: users must manage what the brain ingests from the UI; the released default should be self-ingestion | yes | founder |
